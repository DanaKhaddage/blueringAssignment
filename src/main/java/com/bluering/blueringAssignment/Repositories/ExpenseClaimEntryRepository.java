...
public interface ExpenseClaimEntryRepository extends JpaRepository<ExpenseclaimentryEntity, Integer> {
    @Query(value = "SELECT DISTINCT ExpClaimEntry.expense_type_id FROM ExpenseClaimEntry ExpClaimEntry JOIN ExpenseClaim ExpClaim ON ExpClaimEntry.expense_claim_id = ExpClaim.id JOIN Employee Emp ON ExpClaim.employee_id = Emp.id  WHERE ExpClaimEntry.id = ?1",nativeQuery = true)
